...
def parse_csv(filepath: str | Path, progress_cb=None) -> list[ParsedEvent]:
    """Return ParsedEvent list from a CSV file."""
    ...
    for i, row in enumerate(rows):
        ...
        # Normalise column names — Event Viewer uses slightly different caps
        row_lower = {k.strip().lower(): v.strip() for k, v in row.items()}

        eid_raw = (
            row_lower.get("event id")
            or row_lower.get("eventid")
            or row_lower.get("id")
            or "0"
        )
        ...
        computer = (
            row_lower.get("computer")
            or row_lower.get("computer name")
            or row_lower.get("source")
            or "-"
        )

        user = (
            row_lower.get("user")
            or row_lower.get("username")
            or row_lower.get("account name")
            or "-"
        )

        record_id = _coerce_int(
            row_lower.get("record id") or row_lower.get("recordid") or "0"
        )
        ...
        # Keep all original columns as raw_fields
